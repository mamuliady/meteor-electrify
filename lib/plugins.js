...
function Plugins($){
  ...
  this.releaseFilePath = path.join(this.$.env.app.bin, 'plugins.json');
  if (fs.existsSync(this.releaseFilePath)) {
    this.$.env.plugins = require(this.releaseFilePath);
  } else {
    this.$.env.plugins = {
      aquiringMeteorRelease: '',
      binVersions: {}
    };
  }
...
Plugins.prototype.auto_load = function(done){
  ...
  this.log.info('load configured plugins', this.$.env.app.config.plugins);

  _.each(this.$.env.app.config.plugins, function(plugin_name){
...
Plugins.prototype.acquire = function(done) {
  ...
  if (env.app.isnt_packaged) {
    env.plugins.aquiringVersionMissmatch = env.plugins.aquiringMeteorRelease !== env.meteor.releaseVersion;

    if (env.plugins.aquiringVersionMissmatch) {
      var originalDone = done;
      done = function() {
        env.plugins.aquiringMeteorRelease = env.meteor.releaseVersion;
        delete env.plugins.aquiringVersionMissmatch;
        fs.writeFileSync(self.releaseFilePath, JSON.stringify(env.plugins, null, 2));
        originalDone();
      };
    }
  }
