...
function _cleanPlatformString(platformString){
  if(platformString.toLowerCase().includes('mingw64_nt') || platformString.toLowerCase().includes('cygwin')) return 'win32';
...
function Env(input, output, settings, cli, buildSettings) {
  ...
  this.sys = {};
  this.sys.platform   = buildSettings.platform || _cleanPlatformString(process.platform);
  this.sys.arch       = buildSettings.arch || process.arch;

  // operational System
  this.os = {
    is_windows : (_cleanPlatformString(process.platform) === 'win32'),
