...
Scaffold.prototype.prepare = function() {
  ...
  if (!fs.existsSync(packageJson)) {
    ...
    const additionalDependencies = projectPackageJson.electrifyDependencies ? electrifyDependencies : {};
    let packageJsonContents = {
      name: projectPackageJson.name || 'my-electrified-app',
      main: 'index.js',
      dependencies: {
        ...additionalDependencies,
        "@megatroncupcakes/meteor-electrify": require(path.join(npmLib, 'package.json')).version
      }
    };
