...
const link = path.join(binDir, 'resources');

if (os.platform() == 'win32') {
  if(!existsSync(binDir)) mkdirSync(binDir);
  if(!existsSync(link) && existsSync(resourcesDir)){      
      const _command = `mklink /J ${link} ${resourcesDir}`;
      const _options = {
          windowsHide: true,
          shell: true
      };
      spawn(_command, _options, error => {
          throw error;
      });
  }        
}
...
app.on('ready', function() {
  ...
  const projectJson = require(path.join(resourcesDir, 'app', 'package.json'));
